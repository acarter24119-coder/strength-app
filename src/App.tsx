...
export default function App() {
  ...
  const [customExercises, setCustomExercises] = useState<Record<WorkoutKey, string[]>>(
  ...
  const getTodaySetCountForExercise = (exerciseName: string) => {
    ...
    return history.filter((s) => s.exercise === exerciseName && s.date.slice(0, 10) === today).length;
  };
  ...
  const goToNextWorkout = () => {
    ...
    setWorkoutKey(next);
    setTodaysExercises([...workouts[next], ...(customExercises[next] || [])]);
    setExercise("");
    setWeight("");
    setReps("");
    setSetNumber(1);
  ...
  const exportCSV = () => {
    ...
    const rows = history.map((s) => `${s.workout},${s.exercise},${s.weight},${s.reps},${s.date}`).join("\n");
    const blob = new Blob([header + rows], { type: "text/csv;charset=utf-8;" });
    saveAs(blob, "Strength-History.csv");
  };

  const todaysSets = history.filter((s) => s.date.slice(0, 10) === getTodayISODate());
  ...
  const todaysByExercise = todaysSets.reduce<Record<string, SetLog[]>>((acc, curr) => {
    if (!acc[curr.exercise]) acc[curr.exercise] = [];
    acc[curr.exercise].push(curr);
    return acc;
  }, {});

  const groupedByExercise = history.reduce<Record<string, SetLog[]>>((acc, curr) => {
    if (!acc[curr.exercise]) acc[curr.exercise] = [];
    acc[curr.exercise].push(curr);
    return acc;
  }, {});
  ...
  Object.entries(todaysByExercise).forEach(([ex, sets]) => {
    const todaysBest1Rm = sets.reduce((max, s) => Math.max(max, calcOneRm(s.weight, s.reps)), 0);
    ...
    return (
      <div className="container">
        ...
        >
          ...
          <div style={{ marginTop: 10, borderBottom: "1px solid #e5e5ea" }} />
        </div>
        ...
        <div className="summary-box fade-in">
          ...
          <div style={{ marginBottom: 20 }}>
            <div><strong>Total sets:</strong> {todaysTotals.sets}</div>
            <div><strong>Total reps:</strong> {todaysTotals.reps}</div>
            <div><strong>Total volume:</strong> {todaysTotals.volume.toFixed(1)} kg</div>
          </div>
        </div>
        ...
        >
          <div style={{ display: "flex", gap: 8 }}>
            <button className="secondary" style={{ flex: 1 }} onClick={() => setShowSummary(false)}>Back</button>
            <button style={{ flex: 1 }} onClick={() => { setShowSummary(false); goToNextWorkout(); }}>Next Workout</button>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container">
          ...
          {exercise && (
            <div className="active-exercise fade-in">
              <strong>{exercise} — Set {setNumber}</strong>
            </div>
          )}
                  ...
                  {customExercises[workoutKey]?.includes(ex) && (
                    <span
                      ...
                      onClick={() => {
                        const updated = {
                          ...customExercises,
                          [workoutKey]: customExercises[workoutKey].filter((x) => x !== ex),
                        };
                        saveCustomExercises(updated);
                        setTodaysExercises([...workouts[workoutKey], ...updated[workoutKey]]);
                      }}
                    >
                      ❌
                    </span>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
              <button
                ...
                onClick={() => {
                  ...
                  const updated = {
                    ...customExercises,
                    [workoutKey]: [...(customExercises[workoutKey] || []), name],
                  };
                  saveCustomExercises(updated);
                  setTodaysExercises([...workouts[workoutKey], ...updated[workoutKey]]);
                }}
              ...
              <button style={{ flex: 1 }} onClick={goToNextWorkout}>Next Workout</button>
            </div>
          </div>
          ...
          <div className="card fade-in">
            ...
            <button onClick={saveSet} style={{ width: "100%" }}>Save Set</button>
          </div>
          ...
          <div className="fade-in">
            ...
            {Object.entries(todaysByExercise).map(([exerciseName, sets], groupIndex) => (
              <div key={exerciseName} className="exercise-group fade-in">
                <div className="exercise-group-header">
                  <div className="exercise-group-header-title">{exerciseName}</div>
                  <button
                    className="secondary"
                ...
                {sets.map((s, index) => {
                  const isLastGroup = groupIndex === Object.entries(todaysByExercise).length - 1;
                  ...
                  return (
                    ...
                    >
                      <div>
                        <span style={{ opacity: 0.6, marginRight: 8 }}>Set {index + 1}</span>
                        <span>{s.weight} kg × {s.reps} reps</span>
                      </div>

                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 12, opacity: 0.6 }}>{s.date.slice(11, 16)}</span>
                        <button className="set-delete-btn" onClick={() => deleteSet(s.id!)}>❌</button>
                      </div>
                    </div>
                  );
                })}
          ...
          >
            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ flex: 1 }} onClick={() => setShowSummary(true)}>Finish Workout</button>
              <button className="secondary" style={{ flex: 1 }} onClick={() => setPage("history")}>History</button>
              <button className="secondary" style={{ flex: 1 }} onClick={exportCSV}>Export CSV</button>
            </div>
          </div>
        </div>
      )}
